...
class Handler 
{
    public:
                ...
                const mbot_imu_t* msg)
        {
            ...
            // Sample output of thing being received by IMU channel

            std::cout << "utime" << msg->utime << endl;
            std::cout << "gyro [0] " << msg->gyro[0] << endl;
        }

    /* **************************TODO ****************
    void handlePHOTOONEMessage(const lcm::ReceiveBuffer* rbuf,
     const std::string& chan, 
     const PHOTOONE_t* msg)
        {
...
/* **************************TODO ****************
void lcm_PHOTOONE_handler(void){
    ...
    if(!lcm.good())
        return;
    Handler handlerObject;
    lcm.subscribe("PHOTO CHANNEL THING ", &Handler::handlePTHOTOONEMessage, &handlerObject);
...
int main(int argc, char *argv[])
{
    ...
    // ===================== IMU CHANNEL THREAD INIT ======================== //
    
    // 1. We make a thread runs concurrently with main. 
    std::thread lcm_thread(lcm_imu_handler);

    /*  ************TODO****************** 
    
        std::thread lcm_thread(lcm_photo_handler);

    ************************************** */


    // 2. Can add code here for now, will update. RN we run infinite loop to stop execution
    //    NOTICE NO EXECUTION PAST THIS
     while(1){}



    // ===================== IMU CHANNEL THREAD INIT  END ======================== //
    ...
    // Join the thread right before finish //
    lcm_thread.join();
    return 0;
}
